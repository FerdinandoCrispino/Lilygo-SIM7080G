...
// $00022123& comando do sensor de distancia - serial RS232
#define RXD2 45
#define TXD2 48
...
Preferences prefs;         // armazenamento do dados de calibração
...
volatile int flagCalibraGiro = 0;
...
const int pinCalibraGiro = 3;                   // pino de interrupção para Calibtacao do Giroscopio

static bool Calibrar = false;                   // variavel controlada por interrpcao do pinCalibraGiro
...
//  String NBiot_username = "teste_não_ira_conectar";  
  ...
  int num_sensorGiro = 2;                       // numero de sensores de deslocamento I2C
  bool GPS_active = false;                       // ativa ou desativa a leitura do GPS ao iniciar 
};
struct Config cfg;

// ****************************************** Fim - Configuracao do sistema **************************************************
...
// ================= CONFIG =================
#define NUM_IMUS 2
uint8_t imuAddresses[NUM_IMUS] = {0x68, 0x69};
//#define NUM_IMUS 1
//uint8_t imuAddresses[NUM_IMUS] = {0x68};



// ================= ARRAY =================
IMUUnit imus[NUM_IMUS];

// ================= INIT =================
void initIMU(IMUUnit &imu, uint8_t address, TwoWire *bus_i2c ) {
    
    if (!imu.mpu.begin(address, bus_i2c)) {
        Serial.print("Erro MPU no endereco: ");
        Serial.println(address, HEX);
        while (1);
    }

    imu.mpu.setAccelerometerRange(MPU6050_RANGE_2_G);
    imu.mpu.setGyroRange(MPU6050_RANGE_250_DEG);
    imu.mpu.setFilterBandwidth(MPU6050_BAND_5_HZ);

    kalmanInit(&imu.kalPitch);
    kalmanInit(&imu.kalRoll);

    imu.accBias[0] = imu.accBias[1] = imu.accBias[2] = 0;
    imu.gyroBias[0] = imu.gyroBias[1] = imu.gyroBias[2] = 0;
...
void calibrateIMU(IMUUnit &imu) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        imu.mpu.getEvent(&a, &g, &t);

        accSum[0] += a.acceleration.x;
        accSum[1] += a.acceleration.y;
        accSum[2] += a.acceleration.z;
    ...
    imu.accBias[0] = accSum[0] / N;
    imu.accBias[1] = accSum[1] / N;
    imu.accBias[2] = (accSum[2] / N) - 9.81;

    imu.gyroBias[0] = gyroSum[0] / N;
    imu.gyroBias[1] = gyroSum[1] / N;
    imu.gyroBias[2] = gyroSum[2] / N;

    Serial.println("Calibracao concluida!");
}

// ================= LEITURA =================
void readIMU(IMUUnit &imu, int index, String &resultPich, String &resultRoll) {

    sensors_event_t a, g, t;
    imu.mpu.getEvent(&a, &g, &t);

    float dt = (millis() - imu.lastTime) / 1000.0;
    imu.lastTime = millis();

    if (dt <= 0 || dt > 0.1) return;

    float ax = a.acceleration.x - imu.accBias[0];
    float ay = a.acceleration.y - imu.accBias[1];
    float az = a.acceleration.z - imu.accBias[2];

    float roll  = atan2(ay, az) * 180 / PI;
    float pitch = atan2(-ax, sqrt(ay * ay + az * az)) * 180 / PI;

    float gyroX = (g.gyro.x - imu.gyroBias[0]) * 180 / PI;
    float gyroY = (g.gyro.y - imu.gyroBias[1]) * 180 / PI;

    float kPitch = kalmanUpdate(&imu.kalPitch, pitch, gyroY, dt);
    float kRoll  = kalmanUpdate(&imu.kalRoll,  roll,  gyroX, dt);
    ...
    Serial.print(" | Nofilter_Pitch: ");
    Serial.print(pitch);
    Serial.print(" | Nofilter_Roll: ");
    Serial.println(roll);
}
...
String getNamespace(int index) {
    return "imu" + String(index);
}

void saveCalibration(IMUUnit &imu, int index) {

    String ns = getNamespace(index);
    prefs.begin(ns.c_str(), false);

    // Accel
    prefs.putFloat("aX", imu.accBias[0]);
    prefs.putFloat("aY", imu.accBias[1]);
    prefs.putFloat("aZ", imu.accBias[2]);

    // Gyro
    prefs.putFloat("gX", imu.gyroBias[0]);
    prefs.putFloat("gY", imu.gyroBias[1]);
    prefs.putFloat("gZ", imu.gyroBias[2]);
    ...
    Serial.printf("IMU %d calibrada e salva!\n", index);
}

void calibrateAndSave(IMUUnit &imu, int index) {
    calibrateIMU(imu);
    saveCalibration(imu, index);
}

bool loadCalibration(IMUUnit &imu, int index) {
    ...
    imu.accBias[0] = prefs.getFloat("aX", 0);
    imu.accBias[1] = prefs.getFloat("aY", 0);
    imu.accBias[2] = prefs.getFloat("aZ", 0);

    imu.gyroBias[0] = prefs.getFloat("gX", 0);
    imu.gyroBias[1] = prefs.getFloat("gY", 0);
    imu.gyroBias[2] = prefs.getFloat("gZ", 0);

    prefs.end();

    Serial.printf("IMU %d calibração carregada!\n", index);
    return true;
}
...
// interrupção de hardware  --------------------
void IRAM_ATTR ISR_setCalibra() {
  unsigned long currentTime = millis();
  if ((currentTime - lastInterruptTime) > debounceDelay) {
    flagCalibraGiro = 1;
    Serial.println("Interrupção de Calibraçã do Giroscopio!"); 
    lastInterruptTime = currentTime; 
  }
}
...
{    
    ...
    for (int i = 0; i < NUM_IMUS; i++) { 
      ...
      payloadGiro += ",\"PITCH" + String(i) + "\":"  + data2send.pitch[i] + ",\"ROLL" +  String(i) + "\":" + data2send.roll[i] ;
    }
    Serial.print(payloadGiro);
    ...
    payload = "{\"DT\":"+ String("\"") + data2send.dt+ String("\"") +              
              ...
              ",\"lat\":" + data2send.lat +
              ",\"lon\":" + data2send.lon +
              ",\"ID\":" + String("\"") + cfg.idSensor + String("\"")+ 
              ",\"DIST\":" + String("\"") + data2send.distancia + String("\"")+
              payloadGiro + 
              " }";

    //          ",\"PITCH\":" + String("\"") + data2send.pitch[0] + String("\"")+ 
    //          ",\"ROLL\":" + String("\"") + data2send.roll[0] + String("\"")+
...
bool saveArq(const char* filename, const estruturaDadosEnsaio& data) {
  ...
  file.println(data.dt+","+data.press_M1[0]+","+data.chip_temp+","+data.per_bateria+","+data.vol_bateria+","+data.chip_RSSI+
               ","+data.lat+","+data.lon+","+cfg.idSensor + ","+data.distancia+","+data.pitch[0]+","+data.roll[0]);
  file.close();
  return true;
} 
...
bool readArq2send(const char* filename) {
  ...
  if (FFat.exists(filename)) {
    ...
    while (file.available()) {
      ...
      int n = sscanf(line.c_str(), "%24[^,],%f,%f,%d,%d,%d,%f,%f,%5[^,],%f,%f,%f",datahora, &pres, &temp, &bat, &vbat, &rssi, &lat, &lon, idSensor, &distancia, &pitch, &roll);
      ...
      data.pitch[0] = String(pitch);
      data.roll[0] = String(roll);
      ...
      //reset Watchdog 
      esp_task_wdt_reset();         
      file.println(data.dt + "," + data.press_M1[0] + "," + data.chip_temp + "," + data.per_bateria + "," + 
                   data.vol_bateria + "," + data.chip_RSSI + "," + data.lat + "," +data.lon + "," + data.idSensor + "," +
                   data.distancia + "," + data.pitch[0] + "," + data.roll[0]);
      delay(1000);
    }
    file.close();
...
{   
    ...
    Serial.begin(115200);
    delay(100);
    Serial2.begin(115200, SERIAL_8N1, RXD2, TXD2);  // RS232
    ...
    */
    ...
    // ==================== Definição do bus de comuniação para o I2C ==================
    I2C_BUS.begin(SDA_PIN, SCL_PIN, 100000);  // (10000 , 100000, 400000)
    ...
    // ================== setup Acelerometro e Giroscopio ============================== 
    for (int i = 0; i < NUM_IMUS; i++) {
      
        initIMU(imus[i], imuAddresses[i], &I2C_BUS);
        
        bool ok = loadCalibration(imus[i], i);
        if (!ok) {
            Serial.printf("IMU %d sem calibração. Iniciando...\n", i);
            delay(3000); // tempo para estabilizar
            calibrateAndSave(imus[i], i);
        }
    }
    
    pinMode(pinNotGo2sleep, INPUT_PULLDOWN);           // pino de controle - impede de ir para o sleep mode     

    pinMode(pinCalibraGiro, INPUT_PULLDOWN);          // ira realizar a calibração do giroscopio
    attachInterrupt(pinCalibraGiro, ISR_setCalibra, RISING); //pino para interrupção para calibracao do giroscopio
...
int read_sensor_dist(){
  ...
  int j = 0;
  while (j < 5) {
      Serial2.println("$00022123&");
      while (Serial2.available()) {
        String sensorDist = Serial2.readStringUntil('\n');  // read the received character
        //Serial.println(sensorDist);
        //sensorDist.trim(); // Opcional: remove \r ou espaços em branco
        if (sensorDist.length() > 0) {
          val_dist = getValue(sensorDist, "$00023335&$000621");
          leituras[j] = val_dist;
          j = j+1;

          //kalDist->input = val_dist;
          //kalmanUpdateDist(kalDist);
          //Serial.print(", KalmanDist:"); Serial.print(kalDist->x_est);
          //Serial.print(", Dist:"); Serial.println(val_dist);          
        }
      }
      delay(100);
  }
  return val_dist;
  //Serial.print(", FiltroDist:"); Serial.println(filtroMediana(leituras));
}
...
{       
    ...
    // ============= FLAG para deletar arquivo de dados temporarios  ========================
    if (flagDeletarArquivo == 1){      
      flagDeletarArquivo = 0;
      deleteArqDados(FFat, cfg.nomeArq);      
    } 
    // ============= FLAG para deletar arquivo de dados historicos  =========================
    if (flagDeletaAllData == 1){      
      flagDeletaAllData = 0;
      deleteArqDados(FFat, cfg.nomeArqAllData);      
    } 
    // ============= FLAG para ler os arquivo de dados historicos  ==========================
    ...
    if (millis() - previousMillisPar >= cfg.tempoAmostra)  {
        ...
        result.distancia = read_sensor_dist();
        Serial.println(result.distancia);  

        // =========================== Leitura do sensor de pressão ==========================                                      
        readSensor(); 
        
        // =========================== leitura do Giroscopio =================================
        Serial.println("Inicia leitura do Giroscopio...");
        
        if (flagCalibraGiro == 1) {
          Serial.println("Realizando calibracao forcada...");
          for (int i = 0; i < NUM_IMUS; i++) {
              calibrateAndSave(imus[i], i);
          }
          flagCalibraGiro = 0;
        ...
        for (int i = 0; i < NUM_IMUS; i++) {
            int j = 0;
            while (j <=200){
                if (millis() - previousMillisPar >= 10) {
                    
                    readIMU(imus[i], i, result.pitch[i], result.roll[i]);
                    
                    //read_sensor_giroscopio(result.pitch[0], result.roll[0]);
                    j++;
                    previousMillisPar = millis();
                }
            }
        ...
        // =========================== leitura do arquivo temporario para envio ===========

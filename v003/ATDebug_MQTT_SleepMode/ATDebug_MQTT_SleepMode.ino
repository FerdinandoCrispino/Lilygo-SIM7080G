...
void confSIM() {
  ...
  modem.sendAT("+CGDCONT=1,\"IP\", \"kiteiot.vivo.com.br\"");
...
bool saveArq(const char* filename, const estruturaDadosEnsaio& data) {
  ...
  file.println(data.dt+","+data.press_M1[0]+","+data.chip_temp+","+data.per_bateria+","+data.vol_bateria+","+data.chip_RSSI+","+data.lat+","+data.lon+","+cfg.idSensor );
  file.close();
  return true;
} 
...
bool readArq2send(const char* filename) {
  ...
  if (FFat.exists(filename)) {
    ...
    while (file.available()) {
      ...
      //reset Watchdog 
      esp_task_wdt_reset();         
      file.println(data.dt +","+ data.press_M1[0] +","+ data.chip_temp +","+ data.per_bateria+","+ data.vol_bateria+","+ data.chip_RSSI+","+data.lat+","+data.lon+","+data.idSensor);
      delay(1000);
    }
    file.close();
